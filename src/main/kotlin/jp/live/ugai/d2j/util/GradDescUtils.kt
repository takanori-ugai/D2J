...
object GradDescUtils {
    fun plotGD(
        ...
        plot += geomLine { x = "x"; y = "y" }
        plot += geomLine { x = "x"; y = "y1" }
        plot += geomPoint { x = "x"; y = "y1" }
        return plot + ggsize(width, height)
    }

    fun showTrace(res: List<Float>, f: (Float) -> Float, manager: NDManager): Plot {
    ...
    fun train2d(trainer: (List<Float>) -> List<Float>, steps: Int): List<Weights> {
    ...
    fun showTrace2d(f: (Float, Float) -> Float, results: List<Weights>): Plot {
        ...
        plot += geomPoint(data = dd, size = 3) { x = "x"; y = "y" }
        plot += geomContour(data = dd3, color = "red", binWidth = 7.0) { x = "x"; y = "y"; z = "z" }
        return plot + ggsize(500, 400)
    }

    fun plotGammas(time: List<Float>, gammas: List<Float>, width: Int, height: Int): Plot {
        ...
        var plot = letsPlot(data = data)
        plot += geomLine { x = "x"; y = "y1" }
        plot += geomLine { x = "x"; y = "y2" }
        plot += geomLine { x = "x"; y = "y3" }
        plot += geomLine { x = "x"; y = "y4" }
        return plot + ggsize(width, height)
    }
}
